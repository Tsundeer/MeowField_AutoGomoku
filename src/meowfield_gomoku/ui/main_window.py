# -*- coding: utf-8 -*-
"""MeowField_AutoGomoku 主窗口（CustomTkinter 深色界面）。

由组合根 meowfield_gomoku.app 装配并调用 run_ui()。
"""
import os
import queue
import sys
...
from ..application.update_checker import check_latest
from ..domain.board import coord_label
from ..infrastructure.engine.ai_factory import find_rapfi_exe
from ..infrastructure.storage.settings_store import SettingsStore

APP_REPO_URL = "https://github.com/Tsundeer/MeowField_AutoGomoku"


def _app_base_dir() -> str:
    """资源根：PyInstaller 打包后为解包目录，源码运行为仓库根。"""
    if getattr(sys, "frozen", False):
        base = getattr(sys, "_MEIPASS", None)
        return base if base else os.path.dirname(sys.executable)
...
class App:
    ...
    def _check_update(self):
        self._log(f"正在检查更新（当前 v{__version__}）…")
        res = check_latest(__version__)
        url = res["url"] or APP_REPO_URL + "/releases"
        if res["has_update"]:
            messagebox.showinfo(
                "发现新版本",
            ...
            messagebox.showinfo("已是最新", f"当前版本 v{__version__} 已是最新。")

    def _open_debug(self):
        from ..infrastructure.storage.settings_store import app_data_dir
        d = app_data_dir() / "debug"
...
def run_ui(player, settings: dict) -> int:
    """组合根入口：创建主窗口并进入消息循环。"""
    root = ctk.CTk()
    App(root, player=player, settings=settings,
